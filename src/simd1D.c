...
// ---------------------------------------------------------
{
  ...
  xx0 = _mm_shuffle_ps(s,s, _MM_SHUFFLE(2,3,0,1));
  DEBUG(display_vfloat32(xx0, "%4.0f", "xx0 =")); DEBUG(puts(""));
  s = _mm_add_ps(xx0,s);

  xx1 = _mm_shuffle_ps(s,s, _MM_SHUFFLE(1,0,3,2) );
...
// ----------------------------------------------------
{
    ...
    for(i=0; i<n; i++) {
         ...
         x1 = _mm_load_ps((float32*) &vX[i+0]);
         x2 = _mm_load_ps((float32*) &vX[i+1]);

         xx0 = vec_left1(x0, x1);
         xx2 = vec_right1(x1,x2);
         // y = vec_add3(xx0,xx2,x1);
         // y = vec_div3(y);
...
// --------------------------------------------------------
{
    ...
    x1 = _mm_load_ps((float32*) &vX[i-1]);
    x2 = _mm_load_ps((float32*) &vX[i+0]);
    xx0 = _mm_shuffle_ps(x1,x2, _MM_SHUFFLE(1,0,3,2));
